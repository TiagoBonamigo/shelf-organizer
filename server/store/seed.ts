// Seed the store with the prototype's sample dataset on first run, so the user
// has something to look at out of the box. Same shapes as the spec; uses
// crypto.randomUUID() for ids.

import { AppState, Box, Cabinet, DEFAULT_SETTINGS, Shelf } from "../types.js";
import { uuid } from "./persistence.js";
...
const RAW_CABINETS: RawCabinet[] = [
  {
    name: "Living room — main wall",
    shelves: [
      { widthMm: 1100, heightMm: 410, depthMm: 320, orientation: "vertical" },
      { widthMm: 1100, heightMm: 330, depthMm: 320, orientation: "vertical" },
      { widthMm: 1100, heightMm: 330, depthMm: 320, orientation: "vertical" },
      { widthMm: 1100, heightMm: 330, depthMm: 320, orientation: "mixed" },
      { widthMm: 1100, heightMm: 240, depthMm: 320, orientation: "horizontal" },
    ],
  },
  {
    name: "Side cabinet",
    shelves: [
      { widthMm: 900, heightMm: 380, depthMm: 280, orientation: "vertical" },
      { widthMm: 900, heightMm: 320, depthMm: 280, orientation: "vertical" },
      { widthMm: 900, heightMm: 320, depthMm: 280, orientation: "mixed" },
      { widthMm: 900, heightMm: 220, depthMm: 280, orientation: "horizontal" },
    ],
  },
  {
    name: "Hallway shelf",
    shelves: [
      { widthMm: 720, heightMm: 320, depthMm: 240, orientation: "vertical" },
      { widthMm: 720, heightMm: 280, depthMm: 240, orientation: "vertical" },
      { widthMm: 720, heightMm: 200, depthMm: 240, orientation: "horizontal" },
    ],
  },
];

type RawGame = [name: string, w: number, h: number, d: number, bggId: number, expansionOf?: string];
...
const RAW_GAMES: RawGame[] = [
  ...
  ["Cthulhu Wars", 510, 305, 130, 139975],
...
// A handful of realistic-data tweaks per the prototype (manual/override/missing).
const TWEAKS: Array<[string, "manual" | "override", Partial<{ w: number; h: number; d: number }> | null]> = [
  ["Codenames: Pictures", "manual", null],
  ["Crokinole board", "manual", null],
  ["Catan", "override", { w: 290, h: 290, d: 85 }],
];
const MISSING_DIMS = ["Sushi Go!", "Skull"];

export function makeSeedState(): AppState {
  const cabinets: Cabinet[] = [];
  const shelves: Shelf[] = [];

  RAW_CABINETS.forEach((rc, ci) => {
    const cabId = uuid();
    const shelfIds: string[] = [];
    rc.shelves.forEach((rs, si) => {
      const sid = uuid();
      shelves.push({
        id: sid,
        cabinetId: cabId,
    ...
    cabinets.push({ id: cabId, name: rc.name, position: ci, shelfIds });
  });

  const byName = new Map<string, Box>();
  const boxes: Box[] = [];
  for (const [name, w, h, d, bggId, expansionOf] of RAW_GAMES) {
    const box: Box = {
      id: uuid(),
      bggId,
      name,
    ...
    if (expansionOf) {
      // Defer resolution.
      box.notes = `__expansion_of:${expansionOf}`;
    }
  }
  for (const b of boxes) {
    if (b.notes?.startsWith("__expansion_of:")) {
      const parentName = b.notes.slice("__expansion_of:".length);
      const parent = byName.get(parentName);
      b.expansionOfBoxId = parent?.id ?? null;
      delete b.notes;
    }
  }
  for (const [name, src, override] of TWEAKS) {
    const b = byName.get(name);
    if (!b) continue;
    b.dimensionsSource = src;
    if (override) b.dimensions = { ...b.dimensions, ...override };
  }
  for (const name of MISSING_DIMS) {
    const b = byName.get(name);
    if (!b) continue;
  ...
  return {
    ...
    layouts: [],
    activeLayoutId: null,
    settings: { ...DEFAULT_SETTINGS, defaultPaddingReserveMm: 20 },
  };
}
